...
export default function MarketCard({
  ...
  const yesOdds =
    total > 0 ? Math.round((yesPool / total) * 100) : 50

  const noOdds = 100 - yesOdds

  const isLive =
    !market.resolved &&
    (nowInSeconds <= 0 || market.endTime > nowInSeconds)

  const isEnded =
    !market.resolved &&
    nowInSeconds > 0 &&
    market.endTime <= nowInSeconds
  ...
  const category = getMarketCategory(
    market.id,
    market.question
  )
  ...
  return (
    ...
    >
      ...
      {market.resolved && (
        <div
          className={styles.resolvedOverlay}
          aria-hidden
        />
      )}
      ...
      <div className={styles.content}>
        ...
        <div className={styles.oddsRow}>
          <div className={styles.oddsYes}>
            <span className={styles.oddsNum}>
              {yesOdds}
              <span className={styles.oddsPct}>%</span>
            </span>

            <span className={styles.oddsLabel}>
              YES
            </span>
          </div>
        ...
        <div className={styles.barTrack}>
          <div
            className={styles.barYes}
            style={{ width: `${yesOdds}%` }}
          />

          <div
            className={styles.barNo}
            style={{ width: `${noOdds}%` }}
          />
        </div>
